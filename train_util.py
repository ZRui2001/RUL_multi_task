...
def get_model(model_config, device, lookback_len=None):
    ...
    if model_name == 'lstm':
        ...
        return DeTransformer(feature_size=lookback_len, **model_config).to(device)
    ...
    elif model_name == 'model_v6':
        return model_v6(**model_config).to(device)

    else:
        raise ValueError(f"Unknown model: {model_name}")


def get_optimizer(optim_name, model, lr, rmsprop_alpha=None):
    if optim_name == 'adam':
        return optim.Adam(model.parameters(), lr)
    elif optim_name == 'rmsprop':
        return optim.RMSprop(model.parameters(), lr, alpha=rmsprop_alpha)
    else:
...
#
#
# def get_loss(model_config, model, X, Y_soh, Y_rul, criterion):
...
#
#
# def train_epoch(model_config, model, train_loader, device, optimizer, criterion_soh, criterion_rul):
#     model.train()
#     train_loss = 0
#     for features, soh, rul in train_loader:
#         features, soh, rul = features.to(device), soh.to(device), rul.to(device)
#         optimizer.zero_grad()
#         loss = get_loss(model_config, model, X, Y_soh, Y_rul, criterion)
#         loss.backward()
#         optimizer.step()
